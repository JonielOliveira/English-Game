...
let acertos: number[][] = [];
let rodadas: number[] = [];
let rodadasExtends: number[] = [];
let round: number = 0;
let qtdRound: number = 0;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  allWords = await loadFileTxt("words");
  allTranslations = await loadFileTxt("translations");
...
//------------------------------------------------------------------
async function loadFileTxt(fileName: string): Promise<string[][]> {
  let matriz: string[][] = [];
  try {
    const response = await fetch(`./assets/data/texts/${fileName}.txt`);
      if (response.ok) {
        const content = await response.text();
        let linhas = content.split('\n').map(linha => linha.trim());
        let elementosLinha: string[] = [];
...
//------------------------------------------------------------------
function assignButtonEvents(rodadasExtends: number[]) {

  buttonsSend = document.querySelectorAll<HTMLButtonElement>(".send-button");
  buttonsAudio = document.querySelectorAll<HTMLButtonElement>(".audio-button");
  textInputWord = document.querySelectorAll<HTMLInputElement>(".text-input-w");
  textInputTranslation = document.querySelectorAll<HTMLInputElement>(".text-input-t");
  ...
  buttonsSend.forEach((button, index) => {
    button.addEventListener("click", (event) => {
      ...
      checkAnswer(textInputWord[index], 
                  allWords[rodadasExtends[index]], 
                  textInputTranslation[index], 
                  allTranslations[rodadasExtends[index]],
                  button,
                  index);
    });
  });
...
//------------------------------------------------------------------
function playAudioSys(word: string) {
...
//------------------------------------------------------------------
async function playWordsSequentially(words: string[][]) {
...
//------------------------------------------------------------------
function checkAnswer(textInputWord: HTMLInputElement, correctWord: string[], textInputTranslation: HTMLInputElement, 
                     correctTranslation: string[], button: HTMLButtonElement, index: number) {
  
  let userWord = textInputWord.value;
  let userTranslation = textInputTranslation.value;
  let question1: boolean = correctWord.indexOf(userWord.toLowerCase()) !== -1;
  let question2: boolean = correctTranslation.indexOf(userTranslation.toLowerCase()) !== -1;
...
// intervalo definido
//------------------------------------------------------------------
function getRandomNumbersInRange(minNumber: number, maxNumber: number): number[] {
  ...
  let numerosNaoSorteados: number[] = [];
...
//------------------------------------------------------------------
function getRandomNumberInRange(minNumber: number, maxNumber: number): number {
  let min = minNumber;
  let max = maxNumber;
  if (minNumber == maxNumber) {
    return minNumber;
...
//------------------------------------------------------------------
function shiftRound(step: number) {
...
function inserirQuestoes (quantidade: number) {
...
function expandirLista(rodadas: number[], fator: number): number[] {
  let lista: number[] = [];
  rodadas.forEach( (n) => {
...
function exibirQuestao(n: number) {
...
function ocultarQuestao(n: number) {
  let questao = document.getElementById(`cont${n}`);
  if (questao) {
...
function iniciarResponses(quantidade: number): boolean[][] {
  let matriz: boolean[][] = [];
  for (let i = 0; i < quantidade; i++) {
...
function createStartScreen(min: number, max:number) {
  ...
  const numRoundsInput = document.getElementById('num-rounds') as HTMLInputElement;
